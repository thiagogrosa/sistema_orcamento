...
processar_demandas.py  -->  gmail_client.py  -->  Gmail API
       |                                              |
       v                                              v
extrator_dados_email.py                      emails-extraidos/{ID}/
       |
       v
...
atualizar_asana.py  -->  Asana API
       |
       v
plano-atualizacao-asana.json (ou execução direta)
```
...
|--------|--------|--------|
| `scripts/processar_demandas.py` | Processamento em lote de demandas | Validado |
| `scripts/extrator_dados_email.py` | Extração de dados via regex | Validado |
| `scripts/atualizar_asana.py` | Atualização de tarefas no Asana | Validado |
...
python scripts/processar_demandas.py

# Processar uma demanda específica
python scripts/processar_demandas.py --id 26_049

# Filtrar por prioridade
python scripts/processar_demandas.py --prioridade alta

# Limitar quantidade
python scripts/processar_demandas.py --limite 5

# Simular sem executar
python scripts/processar_demandas.py --dry-run
```

### Extrair dados de emails já baixados
```bash
# De um arquivo
python scripts/extrator_dados_email.py email.txt

# De um diretório
python scripts/extrator_dados_email.py emails-extraidos/26_049/
```
...
python scripts/atualizar_asana.py --id 26_049
```

A execução do plano é feita via **MCP Asana no Claude Code** (interativo):
```
"Atualize as tarefas no Asana conforme plano-atualizacao-asana.json"
```
...
- Emails salvos em `emails-extraidos/26_049/`
