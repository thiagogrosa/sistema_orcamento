...
cp automation/config.json.example automation/config.json

# Editar conforme necessário
nano automation/config.json
```
...
0 9,14,17 * * * cd $PROJECT_DIR && source venv/bin/activate && python automation/scheduler.py job-completo >> automation/logs/cron.log 2>&1

# Sincronizar Drive a cada hora
0 * * * * cd $PROJECT_DIR && source venv/bin/activate && python automation/scheduler.py sync-drive >> automation/logs/cron_sync.log 2>&1
```
...
0 9,15 * * * cd $PROJECT_DIR && python automation/scheduler.py processar-novas

# Sincronizar 1x por dia
0 18 * * * cd $PROJECT_DIR && python automation/scheduler.py sync-drive
```
...
0 9,14,17 * * * cd $PROJECT_DIR && python automation/scheduler.py job-completo
```
...
0 8-18/2 * * 1-5 cd $PROJECT_DIR && python automation/scheduler.py job-completo

# Sincronizar a cada hora
0 * * * * cd $PROJECT_DIR && python automation/scheduler.py sync-drive
```
...
0 22 * * * cd $PROJECT_DIR && python automation/scheduler.py job-completo
```
...
```bash
# Log do dia
tail -f automation/logs/scheduler_$(date +%Y%m%d).log

# Logs de cron
tail -f automation/logs/cron.log

# Últimas 50 linhas
tail -50 automation/logs/scheduler_$(date +%Y%m%d).log
...
# Adicionar ao cron
0 0 * * 0 find $PROJECT_DIR/automation/logs -name "*.log" -mtime +30 -delete
```
...
35 14 * * * cd $PROJECT_DIR && python automation/scheduler.py job-completo
...
0 9 * * * /full/path/to/venv/bin/python /full/path/to/automation/scheduler.py job-completo

# Ou ativar venv explicitamente
0 9 * * * cd $PROJECT_DIR && source venv/bin/activate && python automation/scheduler.py job-completo
```
...
cd /path/to/project && source venv/bin/activate && python automation/scheduler.py job-completo
```
...
# Adicionar ao cron:
0 0 * * 0 find $PROJECT_DIR/automation/logs -name "*.log" -mtime +7 -delete
...
- **Logs:** `tail -f automation/logs/scheduler_*.log`

### Documentação

- **Scheduler:** Ver código em `automation/scheduler.py`
...
1. Verificar logs: `automation/logs/`
2. Testar manualmente
