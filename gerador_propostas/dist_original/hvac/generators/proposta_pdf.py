...
def gerar_proposta_pdf(
    ...
    # Gera numero do orcamento se nao informado
    if numero_orcamento is None:
        numero_orcamento, seq = proximo_numero_orcamento(configs)

    # Detecta revisao
    cliente_dir = criar_pasta_cliente(cliente_nome)
    revisao, _ = detectar_revisao(cliente_nome, cliente_dir)

    # Atualiza numero com revisao se necessario
    if "-R" in numero_orcamento:
        numero_base = numero_orcamento.rsplit("-", 1)[0]
        numero_orcamento = f"{numero_base}-{revisao}"
    else:
        numero_orcamento = f"{numero_orcamento.replace('-R00', '')}-{revisao}"
    ...
    contexto = {
        ...
        "cliente": {
            "razao_social": dados_cliente.get("razao_social", cliente_nome),
...
def main():
    ...
    parser.add_argument("--numero", "-n", help="Numero do orcamento")
